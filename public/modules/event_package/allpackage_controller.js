...
angular.module('alisthub').controller('allpackageController', function($scope,$localStorage,$injector, $uibModal,$rootScope, $filter,$timeout,$sce,$location, $ocLazyLoad,$state,ngTableParams) { 
    
    $rootScope.class_status=false;
    var eventService = $injector.get("event_package");
    $scope.eventloader = false;
   
    $scope.seller_id = $localStorage.userId;
   
    //upcomming event list
    $rootScope.getAllPackageEvent = function(allevent) {
      ...
      eventService.getAllPackageEvent({ 'user_id' : $localStorage.userId , 'allevent' : allevent, 'dateRange': dateRange },function(response) {
        $scope.eventloader = false;
        if (response!=null) {
