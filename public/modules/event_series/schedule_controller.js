...
angular.module('alisthub', ['google.places', 'angucomplete']).controller('schedule_controller', function($scope,$localStorage,$injector, $uibModal,$rootScope, $filter,$timeout,$sce,$location) { 
   //For Step 1
    ...
    $scope.savedata=function(data) {
        if (data.eventtype=='single') {
          if (($scope.selectevent_date!=undefined) &&($scope.startevent_time!=undefined)&&($scope.endevent_time!=undefined)) {
            ...
            $serviceTest.add_schedule(data,function(response){
          ...
          data.userId=$localStorage.userId;
          $serviceTest.saverecurringschedule({'data':data,'date':$scope.between_date},function(response){
            if (response.code == 200) {
              $scope.success="Event Successfully Saved.";
              $scope.data={};
              $scope.error_message=false;
